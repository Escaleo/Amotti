<div class="w-full lg:w-1/3 bg-orange-500 rounded-[2rem] p-8 flex flex-col justify-center items-center shadow-xl">
    ...
    <form action="{{ route('contact.send') }}" method="POST" class="w-full space-y-4 text-black font-light">
        @csrf
        ...
        <input type="text" name="nombre" placeholder="Nombres"
            ...
            @error('nombre') aria-invalid="true" @enderror value="{{ old('nombre') }}" required>
        @error('nombre')
            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
        @enderror

        <!-- Teléfono -->
        <input type="tel" name="telefono" id="telefono" data-phone
            ...
            @error('telefono') aria-invalid="true" @enderror value="{{ old('telefono') }}" required>
        @error('telefono')
            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
        @enderror
        ...
        <input type="email" name="email" placeholder="E-mail"
            ...
            @error('email') aria-invalid="true" @enderror value="{{ old('email') }}" required>
        @error('email')
            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
        @enderror
        ...
        <input type="text" name="web" placeholder="Página Web"
            ...
            @error('web') aria-invalid="true" @enderror value="{{ old('web') }}">
        @error('web')
            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
        @enderror

        <!-- Mensaje -->
        <textarea name="mensaje" placeholder="Mensaje" rows="2"
        ...
        <div class="flex items-start gap-3 text-sm mt-4 text-white">
            <input type="checkbox" name="privacidad" id="privacidad"
                ...
                @error('privacidad') aria-invalid="true" @enderror {{ old('privacidad') ? 'checked' : '' }} required />
